...
from .wiresorts import ToSync, ToPort, FromSync, FromPort
...
class Module(object):
    ...
    def __init__(self, name="", block=None, timing=None, assume_ascriptions=False):
        ...
        annotate_module(self, assume_ascriptions)
    ...
    @classmethod
    def from_block(cls, block=None, timing=None, assume_ascriptions=False):
    ...
    @classmethod
    def from_subcircuit(cls, subckt, timing=None, src_map=None):
        self = cls.__new__(cls)
        self.class_name = subckt.model['model_name']  # annotate_module() needs this
        self.inputs = set()
        self.outputs = set()
        self.submodules = set()
        self.inputs_by_name = {}  # map from input.original_name to wire (for performance)
        ...
        self.supermodule = None
        self.wires = set()
        self.name = next_mod_name()
        self.block = subckt.block

        self.block._add_module(self)

        if src_map is None:
            # Hackingly trying to cache this for all the recursive calls
            src_map, _ = self.block.net_connections()

        self._in_definition = True
        self.block._current_module_stack.append(self)

        # I *really* don't want to go through and replace the
        # wire. Instead, just change its type, and hope this works!
        for i in subckt.inputs.values():
            i.__class__ = _ModInput
            assert hasattr(i, '_original_name')
            i.sort = None
            i.module = self
            self.inputs.add(i)
            self.inputs_by_name[i._original_name] = i
        for o in subckt.outputs.values():
            o.__class__ = _ModOutput
            assert hasattr(o, '_original_name')
            o.sort = None
            o.module = self
            self.outputs.add(o)
            self.outputs_by_name[o._original_name] = o

        for sc in subckt.subckts:
            mod = Module.from_subcircuit(sc, src_map=src_map, timing=timing)
            mod.supermodule = self
            self.submodules.add(mod)
            self.submodules_by_name[mod.name] = mod
        ...
        import timeit
        ts = timeit.default_timer()
        # NOTE: it's *extremely* important that annotation isn't done until submodules
        # have been done. Thus the recursive nature of this from_subcircuit, descending
        # until we can no farther before annotating, so submodules are done before the
        # supermodule using them is annotated.
        annotate_module(self, src_map=src_map)
        te = timeit.default_timer()
        if timing is not None:
            timing[self.class_name] = {'start': ts, 'end': te}
        return self

    # NOTE: Might consider changing this for simplicity, so that the wire type remains
    # a wirevector, with an internal flag saying it's an input to which module.
    ...
    def to_mod_input(self, wire, name=None, sort=None):
        """ Converts a wire into one of this module's inputs """
        name = name if name else wire.name
        new_wire = self.Input(len(wire), name=name, sort=sort)
        replace_wire(wire, new_wire, new_wire, self.block)
        return new_wire

    def to_mod_output(self, wire, name=None, sort=None):
        """ Converts a wire into one of this module's outputs """
        name = name if name else wire.name
        new_wire = self.Output(len(wire), name=name, sort=sort)
        replace_wire(wire, new_wire, new_wire, self.block)
        return new_wire
...
def module_from_block(block=None, timing=None):
    return Module.from_block(block, timing=timing)


def module_from_subcircuit(subckt, timing=None):
    return Module.from_subcircuit(subckt, timing=timing)


def module_from_function(f):
    pass
...
class _ModInput(_ModIO):
    ...
    def __init__(self, bitwidth, name, module, sort=None):
        """ Sort can be the type, or an instance of the object with the output port set filled in
...
class _ModOutput(_ModIO):
    ...
    def __init__(self, bitwidth, name, module, sort=None):
        """ Sort can be the type, or an instance of the object with the input port set filled in
