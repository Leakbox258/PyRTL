...
class TestFauxify(unittest.TestCase):
    ...
    def test_simulation_two_outputs(self):
        ...
        i1, values1 = utils.an_input_and_vals(10)
        i2, values2 = utils.an_input_and_vals(10)
        o1, o2 = pyrtl.output_list('o1/10 o2/10')

        pyrtl.fauxify(div_mod, [i1, i2], [o1, o2])

        outvals = utils.sim_and_ret_outws([i1, i2], [values1, values2])
        expected_o1_outs = [a // b for a, b in zip(values1, values2)]
        expected_o2_outs = [a % b for a, b in zip(values1, values2)]
