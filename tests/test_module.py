...
class TestModIO(unittest.TestCase):
    ...
    # This *should* be illegal (but currently is not)
...
class TestModuleSynthesis(unittest.TestCase):
    def setUp(self):
        pyrtl.reset_working_block()

    def test_synthesize_single_module(self):
        class A(pyrtl.Module):
            def definition(self):
                a = self.Input(4, 'a')
                b = self.Input(4, 'b')
        ...
        self.assertEqual(len(block.wirevector_subset(cls=pyrtl.Input)), 2)
        self.assertIsInstance(block.wirevector_by_name['a'], pyrtl.Input)
        self.assertIsInstance(block.wirevector_by_name['b'], pyrtl.Input)

        self.assertEqual(len(block.wirevector_subset(cls=pyrtl.Output)), 1)
        self.assertIsInstance(block.wirevector_by_name['c'], pyrtl.Output)

        sim = pyrtl.Simulation()
        for i, j in [(0, 2), (4, 6), (1, 9), (11, 15)]:
            sim.step({'a': i, 'b': j})
            self.assertEqual(i * j + 5, sim.inspect('c'))

    def test_synthesize_module_hierarchy(self):
        class A(pyrtl.Module):
            def definition(self):
                a = self.Input(4, 'a')
                b = self.Input(4, 'b')
        ...
        block = pyrtl.working_block()
        self.assertEqual(len(block.wirevector_subset(cls=pyrtl.Input)), 1)
        self.assertIsInstance(block.wirevector_by_name['d'], pyrtl.Input)

        self.assertEqual(len(block.wirevector_subset(cls=pyrtl.Output)), 1)
        self.assertIsInstance(block.wirevector_by_name['e'], pyrtl.Output)

        sim = pyrtl.Simulation()
        sim.step_multiple({'d': [1, 2, 3, 4, 5, 6]})
        pyrtl.synthesize()

        output = six.StringIO()
        sim.tracer.print_trace(output, compact=True)
        self.assertEqual(output.getvalue(), 'd 123456\ne 810121402\n')
